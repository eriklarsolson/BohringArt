...
export const SixGridContainer: React.FC<GridContainerProps> = ({objectiveImage, showGrid}) => {
    ...
    const [currentComp, setCurrentComp] = useState<{x: number, y: number, type: string, voltage: number}>(
        {x: 0, y: 0, type: ComponentTypes.BATTERY, voltage: 1})

    const [totalVoltage, setTotalVoltage] = useState<number>(getTotalVoltage())

    const didItPass = (component: {x: number, y: number, type: string, voltage: number}) => {
                            ...
                            <Row>
                                ...
                                <Col>
                                    <VoltageSlider aria-labelledby="volt-slider" step={1}
                                                   marks min={0} max={10}
                                                   value={getCurrentX() !== -1 && getCurrentComponent().voltage}
                                ...
                                <Col className={"col-1"}>
                                    <p style={{color: "#29405B", fontWeight: "bold"}}>
                                        {getCurrentX() !== -1 && getCurrentComponent().voltage}v
                                    </p>
                                </Col>
                            </Row>
                        </Container>
